...
class problem(BaseOCClass.super_problem):
    ...
    def __init__(self):
        ...
        self.state_labels = [r"$y$", r"$S$", r"$R$", r"$t$"]
        self.control_labels = [r"$u_1$", r"$u_2$"]
    ...
    def get_ode(self):
        ...
        # Model equations
        ydot = cs.exp(-rho * t) * (U - A - u1 * C - D)
        Sdot = u1 - u2 - gamma * (S - omega * D_L)
        Rdot = -u1
        xdot = cs.vertcat(ydot, Sdot, Rdot)
        # Objective term
        L = 0
    ...
    def get_init(self):
        init = {}
        init["s_start"] = cs.DM([0., 2.e3, 1.e4])
        init["q_start"] = [10., 10.]
