...
from utils.get_problem import get_problem
...
class OCGUI(GUIBaseClass.GUI):
    def __init__(self):
        super().__init__()

    def set_opt_entries(self):
        self.options = ["Van der Pol",
                        "Lotka Volterra",
                        "Batch Reactor",
                        "Bioreactor",
                        "Bryson Denham",
        ...
        self.init_types = ["automatic",
                           "linear",
                           "random"]
        self.lift_options = ["all",
                             "none",
                             "adaptive",
                             "last",
                             "all but last",
    ...
    def define_vars(self):
        ...
        self.penalty_solver = tk.BooleanVar()
        self.penalty_parameter = tk.DoubleVar()
    ...
    def insert_options(self):
        ...
        # Use Mayer formulation
        tk.Checkbutton(master=self.left_frame,
                       text='Mayer term version',
                       variable=self.mayer_term,
                       onvalue=True,
                       offvalue=False
    ...
    def plot(self):
        # initialize problem
        if self.mayer_term.get():
            try:
                curr_problem = get_problem(self.problem_name.get() + " Mayer")
            except Exception:
                curr_problem = get_problem(self.problem_name.get())
        else:
            curr_problem = get_problem(self.problem_name.get())
        ...
        # determine end bounds if available
        end_out, _, end_bounds = curr_problem.end_bounds(cs.DM.zeros(s_dim))
        if end_bounds != []:
            if cs.inf not in end_bounds and -cs.inf not in end_bounds:
                if end_out.shape[0] == s_dim:
                    init_vals["s_end"] = cs.DM(end_bounds)
        else:
        ...
        match curr_lifting_type:
            ...
            case "all but last":
                lifting_points = [1 for i in range(num_lifts)] + [0]
            case "sensitivity":
                lifting_points = [1 for i in range(num_lifts + 1)]
    ...
    def solve(self):
        ...
        curr_init_type = self.adapt_init_type(self.init_type.get())
        gui_items = [self.toolbar, self.canvas, self.fig]
